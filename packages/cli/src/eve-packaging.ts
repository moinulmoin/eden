...
export interface EveProjectImage {
  ...
  readonly entrypoint: readonly ["node", ".output/server/index.mjs"];
...
async function assertRequiredRootInputs(root: string): Promise<void> {
  ...
  const lockDetails = await lstat(join(root, "pnpm-lock.yaml")).catch(() => undefined);
  if (lockDetails === undefined) {
    throw new EvePackagingError({
      code: "DEPENDENCY_AMBIGUITY",
      subject: "pnpm-lock.yaml",
      reason: "The selected Eve root is missing its required root pnpm-lock.yaml.",
      remediation: "Add the matching root lockfile and retry with the declared exact pnpm version.",
    });
  }
  if (!lockDetails.isFile() || lockDetails.isSymbolicLink()) {
    throw new EvePackagingError({
      code: "DEPENDENCY_AMBIGUITY",
      subject: "pnpm-lock.yaml",
      reason: "The selected Eve root lockfile must be a regular file.",
      remediation: "Copy the matching lockfile into the selected root and retry.",
    });
  }
}
...
async function prepareOutputParentChain(
  ...
  const canonicalBase = await realpath(
    missingStart === -1
      ? absolute
  ...
  if (missingStart === -1) return canonicalBase;
...
async function scanGeneratedOutput(
  ...
  const entryStable = await readStableFile(entrypoint);
  return {
...
export async function buildEveProjectSnapshot(
  ...
  try {
    ...
    await writeJson(runtimeManifestPath, {
      ...
      platform: "linux/amd64",
      image: {
        id: buildResult.imageId ?? null,
        reference: buildResult.imageReference ?? null,
        digest: buildResult.imageDigest ?? buildResult.imageId ?? null,
      },
      runtimeVariableNames,
    });
    writtenPaths.push(runtimeManifestPath);

    const image: EveProjectImage = {
      ...
      runtimeImage: nodeImage === undefined
        ? null
        : `${nodeImage.reference}@${nodeImage.digest}`,
      imageId: buildResult.imageId ?? null,
      imageReference: buildResult.imageReference ?? null,
      imageDigest: buildResult.imageDigest ?? buildResult.imageId ?? null,
      entrypoint: ["node", ".output/server/index.mjs"],
      workingDirectory: "/app",
    ...
    if (
      ...
      const imageError = new EvePackagingError({
        code: "DOCKER_PLATFORM_BLOCKED",
        subject: "linux/amd64 image",
        reason: "The local Eve candidate did not include a verified Linux/amd64 image identity.",
        remediation: "Build and inspect the final image before handing it to deployment.",
      });
      const cleanupError = await discardBuiltCandidate();
    ...
    return {
      ...
      toolchain: {
        ...
        eveVersion: buildResult.eveVersion ?? eve.version,
